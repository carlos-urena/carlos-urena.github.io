...
___
<center>
<a href="#f1">Figure 1.</a>. <i>A view of a planetoid</i>
...
## Index.
   ...
   1. [High Resolution Sphere](#S1).
       1. [Icosahedron generation](#SS11).
       2. [Triangles subdivisions](#SS12).
   2. [Vertex displacement](#S2).
       1. [Normalization of heights](#SS21).
       2. [Fractal 3D Perlin noise function](#SS22).
       3. [The basic noise functions $$M_i$$](#SS23).
       4. [The 1D interpolation function $$f$$.](#SS24).
...
### <a name="SS11">1.1.</a> Icosahedron generation.
...
### <a name="SS21">2.1.</a> Normalization of Heights

The noise function $$N$$ (explained below) produces a scalar value $$N(\cp)$$ in $$[0,1]$$ for any point $$\cp$$ with coordinates in the range $$[-1,+1]$$ (as the original sphere points have coordinates in that range). We use $$N$$ to compute a height value $$h_i=N(\cv_i)$$ for each vertex position $$\cv_i$$. After that, another scalar value $$d_i$$ is computed from $$h_i$$, so $$d_i$$ values are inside a known range, not necessarily equal to $$[0,1]$$, which can be fitted to specific planetoid characteristics.
...
The following code computes $$f_i$$ and $$d_i$$ values, and displaces the vertexes radially, by using parameters $$c$$ (`p.d_base`) and $$a$$ (`p.d_scale`):
...
### <a name="SS22">2.2.</a> Fractal 3D Perlin noise function.
...
In the image below, we can see the difference between adding all the octaves from 0 to 9 (left) and just adding from octaves 2 to 9 (right). The left planetoid is elongated, as the first two octaves introduce low-frequency variations which make the planetoid depart from a more spherical shape, as in the planetoid on the right.
...
<center>
<img src="imgs2/oct-2-9-min-0_4-hr.png" width="70%"/>
</center>
...
### <a name="SS24">2.4.</a>  The 1D interpolation function $$f$$.
