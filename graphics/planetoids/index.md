...
Regular icosahedron 12 vertexes can be generated by considering their spherical coordinates as detailed in [[2]](#2). Thus, we can enumerate

* A _north pole_ vertex at coordinates $$(0,1,0)$$ (index $$0$$ vertex)
* A _south pole_ vertex at coordinates $$(0,-1,0)$$ (index $$1$$ vertex)
* Five _lower ring_ vertexes, forming a regular pentagon at latitude $$-\arctan(1/2)$$ (indexes $$2$$ to $$6$$). The longitude angle is $$2\pi\,i/5$$, with $$i$$ ranging from $$0$$ to $$4$$. 
* Five _upper ring_ vertexes, forming a regular pentagon at latitude $$+\arctan(1/2)$$ (indexes $$7$$ to $$11$$). The longitude angles are shifted so each lower ring vertex is between two upper ring vertexes. Thus, they can be generated by using longitude angles $$2\pi\,(i+1/2)/5$$, for each $$i$$ from $$0$$ to $$4$$.


The C++ code to generate the vertex table (a `std::vector` of 3-float tuples, named `vertices`) is this:

```cpp 
   const float h = 1.0f/sqrt(5.0f),  // y coordinate for south and north rings (in absolute value)
