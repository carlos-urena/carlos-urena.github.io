import { Assert, glsl } from "./utilidades.js";
...
import { ComprErrorGL } from "./utilidades.js";
import { CMat4, Vec3 } from "./vec-mat.js";
// -------------------------------------------------------------------------
const src_vertex = glsl `#version 300 es
...
`;
// -------------------------------------------------------------------------
const src_fragment = glsl `#version 300 es
    precision highp float;
    precision highp int;
...
export class CauceSombras extends CauceBase {
    fbo_opc = null;
    dir_vista = new Vec3([0, 0, 1]); // dirección de vista
    get fbo() {
    ...
    constructor(gl, sizex, sizey) {
        super(gl);
        const fname = "CauceSombras.constructor:";
    ...
    // --------------------------------------------------------------------------
    fijarDimensionesFBO(nuevo_tamX, nuevo_tamY) {
        const fname = "CauceSombras.fijarDimensionesFBO:";
     ...
     */
    fijarDireccionVista(nueva_dir) {
        const fname = "CauceSombras.fijarDireccionVista:";
        const ld = nueva_dir.length;
