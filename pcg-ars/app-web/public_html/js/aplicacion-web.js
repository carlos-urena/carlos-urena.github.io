import { Log, ComprErrorGL, Assert, EstadoRaton, Ejes, } from "./utilidades.js";
import { CrearCauce } from "./cauce.js";
import { CamaraOrbital3D, CamaraVista2D, Viewport } from "./camaras.js";
import { Vec3, Vec3DesdeColorHex } from "./vec-mat.js";
...
export class AplicacionWeb {
     ...
     */
    input_boton_sombras_arrojadas = null;
    /**
     * Elemento HTML de tipo 'input' (checkbox) para el botón de activar/desactivar sombras arrojadas
     */
    input_boton_visualizar_fbo_sombras = null;
    /**
     ...
     */
    static valor_inicial_param_S = 0.1;
    /**
     * Valor actual del parámetro S
     */
    param_S = AplicacionWeb.valor_inicial_param_S;
    /**
     ...
     */
    material_defecto = new Material(0.1, 0.5, 0.7, 20.0);
    /**
     ...
     */
    constructor() {
        const nombref = "AplicacionWeb.constructor:";
     ...
     */
    async inicializar() {
        const nombref = "AplicacionWeb.inicializar:";
    ...
    async crearObjetosCamaras() {
        ...
        this.camaras.push(new CamaraOrbital3D());
        this.objetos.push(new GrafoTest2(await Textura.crear("/imgs/uv-checker-1.png"), await Textura.crear("/imgs/uv-checker-2.png"), await Textura.crear("/imgs/uv-checker-1.png")));
        this.camaras.push(new CamaraOrbital3D());
        ...
        this.camaras.push(new CamaraOrbital3D());
        this.objetos.push(new OC_GrafoTest2(await Textura.crear("/imgs/uv-checker-1.png"), await Textura.crear("/imgs/uv-checker-2.png"), await Textura.crear("/imgs/uv-checker-1.png")));
        this.camaras.push(new CamaraOrbital3D());
     ...
     */
    crearCheckboxAristas() {
        const nombref = 'AplicacionWeb.crearCheckboxAristas';
        this.input_boton_aristas = CrearInputCheckbox(this.controles, this.visualizar_aristas, 'id_boton_aristas', 'Visu.&nbsp;aristas');
     ...
     */
    crearCheckboxEjes() {
        const nombref = 'AplicacionWeb.crearCheckboxEjes';
        this.input_boton_ejes = CrearInputCheckbox(this.controles, this.visualizar_ejes, 'id_boton_ejes', 'Visu.&nbsp;ejes');
     ...
     */
    crearCheckboxNormales() {
        const nombref = 'AplicacionWeb.crearCheckboxNormales';
        this.input_boton_normales = CrearInputCheckbox(this.controles, this.visualizar_normales, 'id_boton_normales', 'Visu.&nbsp;normales');
     ...
     * Crea el check box para visualizar aristas si/no 'this.input_boton_normales'
     */
    crearCheckboxIluminacion() {
        const nombref = 'AplicacionWeb.crearCheckboxIluminacion';
        this.input_boton_iluminacion = CrearInputCheckbox(this.controles, this.iluminacion, 'id_boton_iluminacion', 'Iluminación');
     ...
     * Crea el check box para visualizar aristas si/no 'this.input_boton_normales'
     */
    crearCheckboxSombrasArrojadas() {
        const nombref = 'AplicacionWeb.crearCheckboxSombrasArrojadas';
        this.input_boton_sombras_arrojadas = CrearInputCheckbox(this.controles, this.evaluar_sombras, 'id_boton_sombras_arrojadas', 'Sombras arrojadas');
     ...
     */
    crearCheckboxVisuFBOSombras() {
        const nombref = 'AplicacionWeb.crearCheckboxVisuFBOSombras';
        this.input_boton_visualizar_fbo_sombras = CrearInputCheckbox(this.controles, this.visualizar_fbo_sombras, 'id_boton_visualizar_fbo_sombras', 'Visu. FBO sombras');
     ...
     */
    crearSelectorObjetoActual() {
        const nombref = 'AplicacionWeb.crearSelectorObjetoActual';
        ...
        this.selector_objeto_actual = CrearSelector(this.controles, this.indice_objeto_actual, 'id_selector_objeto_actual', 'Objeto&nbsp;act.', nombres);
        this.selector_objeto_actual.onchange = (e) => this.fijarObjetoActual(parseInt(e.target.value));
    }
    // -------------------------------------------------------------------------
    /**
     ...
     */
    fijarParamS(nuevo_param_s) {
        const nombref = 'AplicacionWeb.fijarParamS:';
     ...
     * @param obj
     */
    actualizarBotonEstadoAnim(obj) {
        let be = this.input_boton_estado_anim;
        let te = document.getElementById("id_boton_estado_anim_span_estado");
        if (be == null)
     ...
     */
    fijarLongitudLuz(long_luz) {
        const nombref = 'AplicacionWeb.fijarLongitudLuz:';
        Assert(this.col_fuentes.length > 0, `${nombref} no hay fuentes de luz en la colección`);
     ...
     */
    fijarLatitudLuz(lat_luz) {
        const nombref = 'AplicacionWeb.fijarLatitudLuz:';
        Assert(this.col_fuentes.length > 0, `${nombref} no hay fuentes de luz en la colección`);
     ...
     */
    crearElementosControles() {
        const nombref = 'AplicacionWeb.crearElementosControles:';
     ...
     *
     */
    redimensionarVisualizar() {
        const nombref = "AplicacionWeb.redimensionarVisualizar:";
     ...
     */
    obtenerElementoContenedor(id_contenedor) {
        const nombref = "AplicacionWeb.obtenerElementoContenedor:";
     ...
     */
    obtenerElementoControles(id_controles) {
        const nombref = "AplicacionWeb.obtenerElementoContenedor:";
        let controles = document.getElementById(id_controles);
        if (controles == null)
     ...
     */
    obtenerCrearElementoCanvas(contenedor) {
        const nombref = "AplicacionWeb.obtenerCrearElementoCanvas:";
        let canvas = null;
        let lista = contenedor.querySelectorAll("canvas");
        if (lista.length == 0) {
     ...
     */
    obtenerContextoWebGL(canvas) {
        const nombref = "AplicacionWeb.obtenerContextoWebGL:";
     ...
     */
    actualizarEstadoObjeto(ahora_ms) {
        let obj = this.objetos[this.indice_objeto_actual];
        if (obj instanceof ObjetoAnimado) {
            let obj_anim = obj;
            if (obj_anim.estado == EstadoAnim.animado) {
    ...
    visualizarFrame() {
        ...
        const nombref = 'AplicacionWeb.visualizarFrame:';
        ...
        const eval_iluminacion = this.iluminacion && !(camara instanceof CamaraVista2D);
        // indica si se debe evaluar las sombras o no
        const eval_sombras = eval_iluminacion && this.evaluar_sombras;
     ...
     */
    fijarColorDefecto(nuevo_color_inicial) {
        const nombref = 'AplicacionWeb.fijarColorDefecto:';
        this.color_defecto = nuevo_color_inicial;
        const msg = `Color por defecto fijado a ${this.color_defecto.toStringPercent()}`;
        this.estado = msg;
        //Log( `${nombref} ${msg}` )
     ...
     */
    fijarVisualizarEjes(nuevo_visualizar_ejes) {
        ...
        const msg = `Visualizar ejes: ${this.visualizar_ejes ? "activado" : "desactivado"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
     ...
     */
    fijarVisualizarAristas(nuevo_visualizar_aristas) {
        ...
        const msg = `Visualizar aristas: ${this.visualizar_aristas ? "activado" : "desactivado"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
     ...
     */
    fijarVisualizarNormales(nuevo_visualizar_normales) {
        ...
        const msg = `Visualizar normales: ${this.visualizar_normales ? "activado" : "desactivado"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
     ...
     */
    fijarIluminacion(nuevo_iluminacion) {
        ...
        const msg = `Iluminación: ${this.iluminacion ? "activada" : "desactivada"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
     ...
     */
    fijarSombrasArrojadas(nuevo_sombras_arrojadas) {
        ...
        const msg = `Sombras arrojadas: ${this.evaluar_sombras ? "activada" : "desactivada"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
     ...
     */
    fijarVisuFBOSombras(nuevo_visualizar_fbo_sombras) {
        ...
        const msg = `Visualizar FBO de sombras arrojadas: ${this.visualizar_fbo_sombras ? "activado" : "desactivado"}`;
        this.estado = msg;
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
    }
    // ------------------------------------------------------------------------------------
    ...
    fgeRatonArrastre(e) {
        ...
        const dh = -0.3 * e.movementX;
        const dv = 0.3 * e.movementY;
        let camara = this.camaras[this.indice_objeto_actual];
        camara.mover(dh, dv);
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
        return false;
    ...
    fgeRatonRueda(e) {
        ...
        const signo = e.deltaY >= 0.0 ? +1.0 : -1.0;
        let camara = this.camaras[this.indice_objeto_actual];
        camara.zoom(signo);
        window.requestAnimationFrame(VisualizarFrameAplicacionWeb);
        return false;
    ...
    static async crear() {
        ...
        let instancia_apl = null;
        try { // crear e inicializar la instancia única 
        ...
        if (instancia_apl == null)
            ...
            try { // inicializar la instancia única 
     ...
     */
    static gestionarError(err, punto_llamada) {
        const nombref = "AplicacionWeb.gestionarError:";
        let descripcion = "(no hay más información del error)";
        console.log(`${nombref} err constructor name == '${err.constructor.name}'`);
        let tipo = "no disponible";
        let clase = err.constructor.name;
