...
export class MallaPLY extends MallaInd {
    ...
    async leer() {
        ...
        let nt = 0; // numero de caras (triángulos) según la cabecera 
        //let verts : Vec3[] = []   // vertices leídos hasta ahora 
        //let tris  : UVec3[] = []  // triángulos leídos hasta ahora
        let tokens = []; // tokens de la linea actual
        let linea; // linea actual
        let estado = 0; // estado: 0 --> en cabecera, 1 leyendo vértices, 2 leyendo caras
        ...
        if (nl == 0)
            throw Error(`${info} el archivo está vacío (no tiene líneas )`);
        if (this.posiciones.length != nv)
            throw Error(`${info} encontrados ${this.posiciones.length} vértices, pero en la cabecera se dice que debe de haber ${nv}`);
        if (this.triangulos.length != nt)
            throw Error(`${info} encontrados ${this.posiciones.length} triángulos, pero en la cabecera se dice que debe de haber ${nt})`);
