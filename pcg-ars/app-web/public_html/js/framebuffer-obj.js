...
export class FramebufferObject {
    ...
    get tamX() { return this.sizex; }
    get tamY() { return this.sizey; }
    /**
     * Constructor
     */
    constructor(gl, sizex, sizey) {
        ...
        gl.texImage2D(gl.TEXTURE_2D, level, iFormat, this.sizex, this.sizey, border, format, type, data);
        ...
        let depth_internal_format = gl.DEPTH_COMPONENT16;
        if (gl instanceof WebGL2RenderingContext) {
        ...
        if (!gl.isFramebuffer(this.fbo_wgl_act)) {
            const msg = `${fname} no se ha podido crear el framebuffer`;
            Log(msg);
            throw new Error(msg);
        }
        ...
        if (status != gl.FRAMEBUFFER_COMPLETE) {
            const msg = `${fname} el estado del framebuffer para sombras no es 'COMPLETE', es: ${status}`;
            Log(msg);
            throw new Error(msg);
        }
